...
## 2026-09-20 — Refinements from a closer code review (B-17 to B-20)

**Context:** After the custom feature (B-12) the core slices were done. B-17 (brands as a table, mirroring B-11's categories), B-18 (creating a category or brand inline from the product form, reversing B-11's "add only from the toolbar dialog" note), B-19 (lucide icons on the close, edit, delete and remove buttons) and B-20 (money handling) are all backlog items added after that point, and each was recorded in `docs/backlog.md` before it was built.

**Tooling & prompts:** B-20 was built in Claude Code (Sonnet 5) from two short prompts: "add a task for us to implement a better way of handling money", then "let's implement it". Claude first wrote the backlog item, including the proposed approach (integer cents in storage, decimal on the wire, one shared `Money` helper), and only implemented it after that go-ahead. [B-17 to B-19 tooling: not visible from this session.]

**What happened:** This step is me making refinements as I review the code in more detail, rather than new feature slices. B-17 to B-19 came from that review (brand modelled like category, inline creation, icons), and B-20 replaced float prices with integer cents: a new migration, a shared `toCents` / `fromCents` helper, an exact `inventoryValue`, and one shared formatter for display. Each item followed the same flow: a backlog entry, then a branch, then `technical-decisions.md` and the README updated in the same change.
