...
- **Money is stored as integer cents** (`products.price_cents`) so sums are exact; the API and the brief's payload still use the decimal `price` (`19.99`). `toCents` / `fromCents` in `packages/shared/src/money.ts` are the only conversion point, and `inventoryValue` is summed in cents. The currency is a single implicit one (USD).
